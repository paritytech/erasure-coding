...
impl TryFrom<MerklePath> for Proof {
    ...
    fn try_from(input: MerklePath) -> Result<Self, Self::Error> {
        Ok(Proof(
            BoundedVec::try_from(input).map_err(|_| Error::InvalidChunkProof)?,
        ))
    }
}
...
impl From<Vec<Vec<u8>>> for ErasureRootAndProofs {
    fn from(chunks: Vec<Vec<u8>>) -> Self {
        ...
        assert!(
            tree[tree.len() - 1].len() == 1,
            "root must be a single hash"
        );
    ...
    pub fn verify_root(&self, root: &ErasureRoot) -> bool {
        ...
        let root_hash =
            self.proof
                .0
                .iter()
                .fold(leaf_hash, |acc, (hash, direction)| match direction {
    ...
    pub fn verify_index(&self) -> bool {
        let (index, _) =
            self.proof
                .0
                .iter()
                .fold((0u16, 0), |acc, (_, direction)| match direction {
